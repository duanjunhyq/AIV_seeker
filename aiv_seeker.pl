...
sub QC_report () {
  ...
  foreach my $items(@$files) {
      ...
      my $libname=$libs[0];
      if($run_cluster) {
        my $shell= "qsub -o $logs -e $logs $exe_path/src/batch_qsub_fastqc.sh $libs[1] $libs[2] $result_dir $libname $threads";
        system($shell);
      }
      else {
        if($libs[1]=~/\.fq$/i) {
        ...
        print("$fastqc -t $threads $dir_raw/$libname\_R1.fq -o $dir_QC\n");
...
sub quality_filtering () {
  ...
  if($run_cluster) {
    ...
    foreach my $items(@$files) {
      ...
      if($run_cluster) {
        if($run_paired_only) {
          ...
          $shell= "qsub -o $logs -e $logs $exe_path/src/batch_qsub_filtering.sh $exe_path $libname $result_dir $trimmomatic $fastq_to_fasta $adaptor $threads $run_paired_only_tag";
        }
        else {
          $shell= "qsub -o $logs -e $logs $exe_path/src/batch_qsub_filtering.sh $exe_path $libname $result_dir $trimmomatic $fastq_to_fasta $adaptor $threads";
        }        
        system($shell);
      } 
    }
    ...
    foreach my $items(@$files) {
      my @libs= split(/\t/,$items); 
      my $libname=$libs[0];
      my $shell= "qsub -o $logs -e $logs $exe_path/src/batch_qsub_stat.sh $exe_path $libname $result_dir";
      system($shell);
    }
...
sub search_by_diamond () {
  ...
  foreach my $items(@$files) {
    ...
    my $libname=$libs[0];
    if($run_cluster) {
      my $shell= "qsub -o $logs -e $logs $exe_path/src/batch_qsub_diamond.sh $diamond $diamond_db $result_dir $libname $threads";
      system($shell);
    }
    else {
      system("conda run -n aiv_seeker-diamond diamond blastx -d $diamond_db -q $dir_fasta_processed/$libname\_raw\.fasta -a $result_dir/tmp/$libname -e 0.1 -p $threads -t $result_dir/tmp --salltitles");
      system("conda run -n aiv_seeker-diamond diamond view -a $result_dir/tmp/$libname\.daa -o $dir_diamond/$libname\.m8");
    }
  }
}
...
sub cluster_AIV_reads_vsearch () {
  ...
  foreach my $items(@$files) {
    ...
    if(-s "$aiv_reads/$libname\_first_round.fa") {
      if($run_cluster) {
        my $shell="qsub -o $logs -e $logs $exe_path/src/batch_qsub_vsearch.sh $exe_path $vsearch $aiv_reads $libname";
        system($shell);
      }
      else {
        system($p1);
...
sub blast_AIV () {
  ...
  foreach my $items(@$files) {
    ...
    if($run_cluster) {
        my $shell="qsub -o $logs -e $logs $exe_path/src/batch_qsub_blast.sh $aiv_reads/$libname\_reads_derep_with_tag.fa $blast_dir_vs_db/$libname\_blastout.m8 $blast_dir_self/$libname\_self.m8 $flu_ref_gene $threads $blastn $makeblastdb";
        system($shell);
    }
    else {
        system($p1);
...
sub debleeding() {
  ...
  foreach my $source(keys %source_all) {
    if($run_cluster) {
      system("qsub -o $logs -e $logs $exe_path/src/batch_qsub_debled.sh $exe_path $dir_combined_seq $source $cluster_identity $dir_debled $overlap_level $threads $vsearch");
    }
    else {
  ...
  foreach my $items(@$files) {
    ...
    if(-s "$dir_chimeric_seq/$libname\_no_chimeric.fa") {
      if($run_cluster) {
        ...
        #print "$shell\n";
        system("qsub  -o $logs -e $logs $exe_path/src/batch_qsub_subtype_raw.sh $exe_path $result_dir $libname $margin $BSR $percent $blast_db_ano $source");
      }
      else {
