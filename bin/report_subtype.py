...
if __name__ == '__main__':
    ...
    sampleList= []
    fileList = [f for f in os.listdir(args.input) if f.endswith('.csv')]
    df_all = pd.DataFrame()
    for file in fileList:
        sampleName = file.split("_")[0]
        sampleList.append(sampleName)
        df_data = pd.read_csv(args.input+"/"+file, sep=',', keep_default_na=False) 
        if df_data.empty:
            continue
        # add raw_size column
    ...
    if(args.report_raw):
        ...
        df_raw_result.reset_index().rename(columns={'index': 'sampleName'}).to_csv(args.output_prefix +"_raw_subtype.csv", index=False)
        ...
        df_unique_result.reset_index().rename(columns={'index': 'sampleName'}).to_csv(args.output_prefix +"_uni_subtype.csv", index=False)
