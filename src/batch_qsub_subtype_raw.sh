...
perl $exe_path/src/parse_m8_BSR.pl -i $blast_dir_vs_db/$libname\_blastout.m8 -s $blast_dir_self/$libname\_self.m8 -d $blast_db_ano -o $cluster_subtype_step1_blast_sorted/$libname\_sorted.txt -m $dir_chimeric_seq/$libname\_no_chimeric.fa
...
if [ -s ${cluster_subtype_step1_blast_sorted}/${libname}\_sorted.txt ]; then
	...
	perl $exe_path/src/getseq_subtype.pl -i $cluster_subtype_step2_subtype/$libname\_subtype.txt -d $dir_chimeric_seq/$libname\_no_chimeric.fa -o $cluster_subtype_step3_seq/$source1
fi